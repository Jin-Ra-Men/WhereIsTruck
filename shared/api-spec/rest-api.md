...
| POST | `/auth/owner/requests/recommendation` | 사장님 권한 추천 기반 심사 요청 생성 | Bearer 토큰 |
| POST | `/auth/owner/requests/payment` | 사장님 권한 유료 즉시 부여 결제 요청 생성 | Bearer 토큰 |
| POST | `/auth/owner/approve-payment` | 결제 완료 웹훅/백오피스 승인 후 owner 즉시 승격 | 내부/관리자 |
...
### POST /auth/owner/requests/recommendation

**요청 body 예시:**

```json
{
  "business_name": "길동 푸드트럭",
  "description": "야시장 중심으로 영업합니다."
}
```
...
### POST /auth/owner/requests/payment
...
모든 `/admin/*` API는 `Authorization: Bearer <Firebase_ID_TOKEN>` 필요 + `role=admin` 계정만 접근 가능합니다.

| 메서드 | 경로 | 설명 | 인증 |
|--------|------|------|------|
| GET | `/admin/users` | 사용자 목록 조회 (role/q/limit/offset 필터) | Bearer (admin) |
| PATCH | `/admin/users/:id/role` | 사용자 역할 변경 (`user`/`owner`/`admin`) | Bearer (admin) |
| GET | `/admin/owners` | 사장님(owner) 계정 목록 조회 | Bearer (admin) |
| GET | `/admin/trucks` | 트럭 목록 조회 (status/owner_id/q 필터) | Bearer (admin) |
| PATCH | `/admin/trucks/:id/status` | 트럭 영업 상태 강제 변경 (`open`/`closed`) | Bearer (admin) |
| GET | `/admin/audit-logs` | 관리자 변경 이력 조회 | Bearer (admin) |
...
[
  {
    ...
    "target_type": "user",
    "target_id": 21,
    "details": { "beforeRole": "user", "afterRole": "owner" },
...
- 모바일/관리자 Flutter 클라이언트에서 위 API를 기준으로 화면·상태관리 연동
- 실시간 이벤트 스펙은 **shared/api-spec/realtime-events.md** 참고
