...
* **2트랙 권한 획득:** (1) 사용자 추천 시스템을 통한 신뢰 기반 권한 획득, (2) 회사에 일정 금액 지불 시 즉시 권한 부여.
...
* **Admin Program:** `Flutter Web` (운영 관리자용 웹 콘솔, 기존 Flutter 재사용)
...
2. **인증 시스템:** 사장님 권한은 **사용자 추천 기반 심사**와 **유료 즉시 권한 부여** 두 가지 방식을 동시에 운영.
3. **비즈니스 모델:** 사장님 권한 유료 부여, 프리미엄 노출 권한, **타겟팅 푸시 알림** 서비스 모델 구축.
4. **관리 콘솔:** 관리자(운영자)가 사용자/사장님 계정, 트럭 영업 상태를 관리하는 **Flutter Web 기반 Admin 프로그램** 운영.

---

## 🧑‍💼 운영 관리자 프로그램 (Admin)
* **목적:** 서비스 운영자가 계정·권한·영업 상태를 직접 관리할 수 있는 운영 도구 제공.
* **형태:** **웹 콘솔(Flutter Web)** 채택.
* **기술 선택 기준 반영:** 새로운 프레임워크 도입 없이 기존 기술(Flutter + NestJS) 재사용으로 구현 속도와 유지보수 효율 확보.
* **비용 관점:** 기존 백엔드 인프라를 재사용해 추가 서버/운영 리소스 증가를 최소화.
* **주요 기능:**  
  - 사장님/일반 사용자 계정 조회·상태 변경  
  - 사장님 권한 요청(추천 기반) 심사  
  - 유료 즉시 권한 부여 결제 완료건 승인/권한 반영  
  - 가게(트럭) 영업 상태 강제 변경, 노출 제어
